...
def center_distance(box_a, box_b, is_sqrt=False):
    """
    计算矩形间的中心点距离 input: (x_min, y_min, x_max, y_max)
    """
...
def diag_distance(x, is_sqrt=False):
    """
    矩形对角线长度 input: (x_min, y_min, x_max, y_max)
    """
...
def aspect(x):
    """
    矩形宽高比 input: (x_min, y_min, x_max, y_max)
    """
    return torch.abs(x[:, 0] - x[:, 2]) / torch.abs(x[:, 1] - x[:, 3])


def iou_loss(output, target, is_transform=True, ignore_zero=True):
    """
    ...
    # yield torch.mean(-torch.log(iou))  # inf会导致模型无法拟合
    return torch.mean(1 - iou)


def distance_iou_loss(output, target, is_transform=True, ignore_zero=True):
    """ DIoU """
    if is_transform:
        target = box_transform(target)  # [seq_len, 4]
        output = box_transform(output)  # [seq_len, 4]
    ...
    # TODO 取最大值还是取平均值?
    return torch.mean(1 - iou + dcn / dbr)


def complete_iou_loss(output, target, is_transform=True, ignore_zero=True):
    """ CIoU """
    if is_transform:
        target = box_transform(target)  # [seq_len, 4]
        output = box_transform(output)  # [seq_len, 4]
    ...
    return torch.mean(1 - iou + dcn / dbr + aph * val)
...
def structure_loss(
        outputs, targets, is_transform=True, ignore_zero=True, pad_idx=0, smoothing=0.1, weight=None):
    ...
    iou_loss_value = batch_mean(
        complete_iou_loss, box_output, box_target, is_transform=is_transform, ignore_zero=ignore_zero)
...
if __name__ == '__main__':
    ...
    b = (
        torch.randint(9, [1, 3]),
        torch.tensor([[[1, 1, 2, 2], [2, 2, 2, 2.1], [1, 1, 0, torch.nan]]], dtype=torch.float64)
    )
