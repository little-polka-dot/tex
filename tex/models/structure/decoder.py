...
class Decoder(nn.Module):
    ...
    def __init__(self, n_vocab, seq_len, d_model, n_head, d_k, layers, tail_layers=1,
        ...
        self.c_decoders = nn.ModuleList([
            attention.DecodeLayer(
                d_model, n_head, d_k, d_ffn=d_ffn, dropout=dropout) for _ in range(tail_layers)
        ])
    ...
    def decode(self, dec_input, enc_value, enc_mask=None):
        ...
        for layer in self.c_decoders:
            cls_x = layer(cls_x, enc_value, slf_mask=slf_m, enc_mask=enc_mask)
        cls_x = self.cls_fc(cls_x)
        for layer in self.b_decoders:
            box_x = layer(box_x, enc_value, slf_mask=slf_m, enc_mask=enc_mask)
    ...
    def greedy_decode(self, dec_input, enc_value, enc_mask=None):
        ...
        for _ in range(self.seq_len):
            cls_x, box_x = self.decode(dec_input, enc_value, enc_mask=enc_mask)
    ...
    def forward(self, dec_input, enc_value, enc_mask=None):
        # dec_input: [batch_size, dec_len]
        # enc_value: [batch_size, enc_len, d_model]
        if self.training:
            return self.decode(dec_input, enc_value, enc_mask=enc_mask)
        else:
            return self.greedy_decode(dec_input, enc_value, enc_mask=enc_mask)
